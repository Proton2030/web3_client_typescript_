...
export default function AffiliateProgram({referralCode}:any) {
  ...
  const fetchLevelUsers = useCallback(async () => {
    try {
		const user = await axios.get(`https://d1sc3hq7fqk6dl.cloudfront.net/api/v1/auth/getuser-byid/${address}`);
    // console.log("---->user from affilatye",user.data.data?.referralCode);
    
      const response = await axios.get(`https://d1sc3hq7fqk6dl.cloudfront.net/api/levelUsers/${user.data.data?.referralCode}/3`);
